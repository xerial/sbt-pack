...
private[pack] object PluginCompat:
  ...
  def toFile(a: HashedVirtualFileRef)(using conv: FileConverter): File = conv.toPath(a).toFile
  def toFileRef(a: File)(using conv: FileConverter): FileRef           = conv.toVirtualFile(a.toPath)
